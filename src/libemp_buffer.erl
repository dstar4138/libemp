...
%% Initialize the buffer. If a linkable process id is returned LibEMP will 
%% link it to the OTP supervision tree. Otherwise it will return ignore.
-callback initialize( Args :: [ term() ] ) -> {ok, libemp_buffer()} |
                                              {ok, pid(), libemp_buffer()} |
                                              {stop, Reason :: term()}.
...
destroy( #libemp_buffer{destroy=Destroy} = Buffer ) -> 
    wrap_extern(Destroy,[]),
